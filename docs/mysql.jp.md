# Elixir + Phoenix Live with MySQL

`MySQL`と`Phoenix Live`のサンプルです。
...
`mysql`ディレクトリ下に新しい`Phoenix Live`プロジェクトを作成することを想定しています。
...
以下のコマンドを実行し、`mysql`ディレクトリ下に`sample`プロジェクトを作成します。

```bash
# create `mysql` directory
mkdir mysql 
# create phoenix project in `mysql` directory
docker run --rm -v $PWD/mysql:/app -w /app \
  -it mix phx.new sample --module Sample \
  --no-install --no-dashboard \
  --live --database mysql
```

`mysql`ディレクトリに`docker-compose.yml`を作ります。
